...
func fanOut(in <-chan int, n int) []<-chan int {
	xc := make([]<-chan int, n) // here we initaliaze the slice with 10 nil channels -> cant do shit with nil channels
	for i := 0; i < n; i++ {
		xc = append(xc, factorial(in))
	}
	// fmt.Println("len of xc:\t", len(xc[:n])) // gives the answer
	// fmt.Println(xc)                          // gives the answer
	return xc[n:]
}
